...
export class WebSocket {
  ...
  onSocketData(key: string, socket: Socket, data: Buffer) {
    ...
    if (frame.Opcode === 0x9) {
      socket.write(this.encodeDataFrame({
        FIN: 1, Opcode: 0xA, Mask: 0, PayloadData: Buffer.from('pong')
      }))
      return
    }
    ...
    if (frame.Opcode === 0x1) {
      //群发
      this.sockets.forEach(s => s.write(
        this.encodeDataFrame(frame)
      ))
      return
    }
  ...
  encodeDataFrame(df: DataFrame): Buffer {
    const p: number[] = [], d = df.PayloadData, len = d.byteLength
