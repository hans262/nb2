...
export function outFile(ctx: Context, staticPath: string) {
  ...
  stream.pipe(res)
  Logger.self.stdlog({
    level: 'file', color: 'green', logPath: ctx.opt.logDir,
...
export function outDir(ctx: Context, staticPath: string) {
  ...
  try {
    ...
    `)
    Logger.self.stdlog({
      level: 'dir_fail', color: 'red', logPath: ctx.opt.logDir,
   ...
   */
  if (dirents.find(d => d.name === ctx.opt.indexFileName)) {
    return staticTask(ctx, join(staticPath, ctx.opt.indexFileName))
  }
  ...
  Logger.self.stdlog({
    level: 'dir', color: 'green', logPath: ctx.opt.logDir,
    msg: staticPath + ' +' + (Date.now() - ctx.startTime) + 'ms'
  })
}
...
export const out404 = (ctx: Context, opt?: {
  ...
  if (ctx.opt.spa && ctx.opt.staticRoot) {
    /**
     ...
     */
    const reactIndexPath = join(ctx.opt.staticRoot, ctx.opt.indexFileName)
  ...
  Logger.self.stdlog({
    level: '404', color: 'red', logPath: ctx.opt.logDir,
...
export function outCache(ctx: Context, staticPath: string) {
  ...
  Logger.self.stdlog({
    level: 'cache', color: 'cyan', logPath: ctx.opt.logDir,
    msg: staticPath + ' +' + (Date.now() - ctx.startTime) + 'ms'
  })
}
...
export function outRange(ctx: Context, opt: {
  ...
  if (range) {
    ...
    stream.pipe(res)
    Logger.self.stdlog({
      level: 'range', color: 'green', logPath: ctx.opt.logDir,
    ...
    res.end()
    Logger.self.stdlog({
      level: 'range_416', color: 'red', logPath: ctx.opt.logDir,
      msg: opt.staticPath + ' +' + (Date.now() - startTime) + 'ms'
    })
  }
}
...
export function outZip(ctx: Context, staticPath: string, encoding: string) {
  ...
  Logger.self.stdlog({
    level: 'zip', color: 'green', logPath: ctx.opt.logDir,
    msg: staticPath + ' +' + (Date.now() - startTime) + 'ms'
  })
}